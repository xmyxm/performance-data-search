...
Promise.all(pageConfigList.map(({ path, pageId }) => {
    ...
    metricList.forEach((metric) => {
        promiseList.push(fetchPerformanceAPIData(1739356860000, 1739358660000, projectId, pageId, metric).then(val => { data[metric] = val }))
        promiseList.push(fetchDurationDistributionAPIData(1739356860000, 1739358660000, projectId, pageId).then(val => { data.second = val }))
        promiseList.push(fetchDimensionDistributionAPIData(1739356860000, 1739358660000, projectId, pageId).then(val => { data.ios = val.ios; data.android = val.android }))
    })
    ...
    try {
        ...
        if (response.data && response.data.code === 10000) {
            ...
            if (result.columns && result.rows) {
                ...
                val = calculateAverage(result.rows.map((item) => {
                    return item[name]
                }))
            }
        }
